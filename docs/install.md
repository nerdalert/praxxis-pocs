...
| MaaS gpt-4o 404 (persistent) | HTTPRoute backendRef was not patched to `praxis-gateway:8080` or MaaS controller reconciled it back | Re-run `demos/maas-praxis/deploy.sh` |
| MaaS gpt-4o 404 (intermittent) | The MaaS controller creates two HTTPRoute rules: rule 0 (`/llm/gpt-4o` path prefix) and rule 1 (`/` + `X-Gateway-Model-Name` header). If both exist, the Envoy router occasionally picks rule 1 and routes to the `gpt-4o` ExternalName service instead of Praxis, returning an empty 404 | `deploy.sh` now removes rule 1 after patching rule 0. If the flake returns after a MaaS controller reconciliation, re-run `deploy.sh` |
